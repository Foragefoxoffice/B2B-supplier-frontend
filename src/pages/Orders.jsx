...
const Orders = () => {
  ...
  const getStatusBadge = (status) => {
    switch (status) {
      ...
      case 'ACCEPTED': return <span className="px-2 py-1 bg-emerald-100 text-emerald-800 text-xs rounded-full font-medium">Accepted</span>;
      case 'REJECTED': return <span className="px-2 py-1 bg-red-100 text-red-800 text-xs rounded-full font-medium">Rejected</span>;
      case 'COMPLETED': return <span className="px-2 py-1 bg-purple-100 text-purple-800 text-xs rounded-full font-medium">Completed</span>;
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                    {isSupplier && order.status === 'SENT' && (
                      <div className="flex gap-2">
                        <button 
                          ...
                          className="text-emerald-600 hover:text-emerald-900 bg-emerald-50 px-2 py-1 rounded"
                        >
                          Accept
                        </button>
                        <button 
                    ...
                    {!isSupplier && order.status === 'ACCEPTED' && (
                      <button 
                        ...
                        className="text-blue-600 hover:text-blue-900 bg-blue-50 px-3 py-1 rounded font-semibold"
                      >
                        Mark Completed
                      </button>
                    )}
